...
Tự động khởi tạo 100 test cases (50 T2I + 50 I2I) thực tế, phong phú và bám sát nghiệp vụ SME Tendoo AI:
...
# 50 Mẫu T2I đa dạng bám sát nhu cầu phong phú của người dùng SME Tendoo
T2I_DATA = [
    # 1. Poster Khuyến Mại & Ưu Đãi (8 cases)
    ...
    # 2. Ảnh Giới Thiệu Sản Phẩm (8 cases)
    ...
    # 3. Banner Khai Trương & Sự Kiện (7 cases)
    ...
    # 4. Ảnh Feedback Khách Hàng (7 cases)
    ...
    # 7. Yêu Cầu Sinh Tự Do & Văn Hóa Việt (7 cases)
...
def build_benchmark_v1_cases():
    ...
    print(f"  📌 Nạp {len(ref_list)} ảnh reference sản phẩm từ manifest.")
    ...
    # 2. Dựng 50 I2I Cases (Đảm bảo chia đều 5 loại edit_type: 10 mỗi loại = 50)
    I2I_PROMPT_PATTERNS = [
        # background_replacement (1-10)
    ...
    # Ghi bản lưu vết v2 ra tendoo_v1
    CASES_V1_DIR.mkdir(parents=True, exist_ok=True)
    with open(CASES_V1_DIR / "t2i.jsonl", "w", encoding="utf-8") as f:
        for c in t2i_cases:
            f.write(json.dumps(c, ensure_ascii=False) + "\n")

    with open(CASES_V1_DIR / "i2i.jsonl", "w", encoding="utf-8") as f:
        for c in i2i_cases:
            f.write(json.dumps(c, ensure_ascii=False) + "\n")

    print(f"🎉 Đã xây dựng 100 cases Benchmark v2 hoàn chỉnh (50 T2I + 50 I2I)!")
